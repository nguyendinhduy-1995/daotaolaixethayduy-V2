...
export const API_CATALOG: ApiCatalogItem[] = [
  ...
  },
  {
    ...
    auth: "Bearer",
    body: ["channel", "templateKey", "leadId|studentId", "to?", "priority?", "variables?", "note?", "suggestionId?", "actionKey?"],
    response: '{"outboundMessage":{"id":"...","status":"QUEUED"}}',
    curl: `curl -sS -X POST http://localhost:3000/api/outbound/jobs -H 'Authorization: Bearer REDACTED' -H 'Idempotency-Key: REDACTED-UUID' -H 'Content-Type: application/json' -d '{"channel":"CALL_NOTE","templateKey":"remind_schedule","leadId":"REDACTED_LEAD","suggestionId":"REDACTED_SUGGESTION_ID","actionKey":"CREATE_CALL_LIST"}'`,
